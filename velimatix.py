import requests, base64, uuid, os, random, json
...
try:
    ...
    print ("\033[1;32mBạn Chưa Tải Thư Viện \n Bắt Đầu Tải ")
...
def control_server(action):
    ...
    response = requests.post('https://dhphuoc207.x10.mx/api/apikey.php', data=data).json()
    try:
        msg = response['msg']
        return msg
    except:
...
def getjob(cookie, nv):
    ...
    try:
        get = requests.get(f'https://tuongtaccheo.com/kiemtien/{nv}/getpost.php',headers=headers).json()
        return get
    except:
        return False
...
def _delay(value):
	while not(value <= 1):
		value -= 0.123
		print(f'''{red}[{luc}DHP07{red}][{vang}DELAY{red}][{luc}{str(value)[0:5]}{red}]''', '               ', end = '\r')
		sleep(0.02)
		print(f'''{red}[{luc}DHP07{red}][{vang}DELAY{red}][{luc}{str(value)[0:5]}{red}]''', '               ', end = '\r')
		sleep(0.02)
		print(f'''{red}[{luc}DHP07{red}][{vang}DELAY{red}][{luc}{str(value)[0:5]}{red}]''', '               ', end = '\r')
...
def countdown(value):
	while not(value <= 1) :
		value -= 0.123
		print(f'''{red}[{luc}DHP07{red}][{vang}COUNTDOWN{red}][{lam}{str(value)[0:5]}{red}]''', '               ', end = '\r')
		sleep(0.02)
		print(f'''{red}[{luc}DHP07{red}][{vang}COUNTDOWN{red}][{lam}{str(value)[0:5]}{red}]''', '               ', end = '\r')
		sleep(0.02)
		print(f'''{red}[{luc}DHP07{red}][{vang}COUNTDOWN{red}][{lam}{str(value)[0:5]}{red}]''', '               ', end = '\r')
...
def login(token):
    try:
        login = requests.post('https://tuongtaccheo.com/logintoken.php',data={'access_token': token})
        user = login.json()['data']['user']
        sodu = login.json()['data']['sodu']
        cookie = 'PHPSESSID='+(login.cookies)['PHPSESSID']
        return user, sodu, cookie
    except:
        try:
            print(login.json ()['mess'])
        except:
            print(f'{red}Kiểm Tra Kết Nối Mạng (không đc sử dụng ip nước ngoài)')
            return False
    ...
    while True:
        if len(listck) == 0:
            ...
            with open('Cookie_FB.txt', 'w') as f:
                json.dump(listck, f)
        for ck in listck:
            loilike, loicamxuc, loicxcmt, loifl, loicmt, loishare = 0, 0, 0, 0, 0, 0
            _info = _Infofb(ck)
            if _info != False:
                uidfb = _info[1]
                tenfb = _info[2]
            ...
            while True:
                ...
                nvlike2 = 1 if '2' in nhiemvu else 0
                nvlike3 = 1 if '3' in nhiemvu else 0
                nvcx1 = 1 if '4' in nhiemvu else 0
                nvcx2 = 1 if '5' in nhiemvu else 0
                nvcxcmt = 1 if '6' in nhiemvu else 0
                nvfl = 1 if '7' in nhiemvu else 0
                nvcmt = 1 if '8' in nhiemvu else 0
                nvshare = 1 if '9' in nhiemvu else 0
				# nvfl = 1 if '3' in nhiemvu else 0
				# nvcx = 1 if '4' in nhiemvu else 0
				# nvcxcmt = 1 if '5' in nhiemvu else 0
				# nvpage = 1 if '6' in nhiemvu else 0
				# nvgroup = 1 if '8' in nhiemvu else 0
                if nvlike == 1:
                    listlike = getjob(cookie, 'likepostvipcheo')
                    if listlike == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Like Vip             ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                        nvlike = 0
                    else:
                        listlike = listlike
                        if 'error' in listlike:
                            print(f'{luc}Hết Nhiệm Vụ Like Vip                            ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                            nvlike = 0
                        else:
                            print(f'{luc}Tìm Thấy {vang}{len(listlike)} {luc}Nhiệm Vụ Like Vip                      ', end = '\r')
                            for x in listlike:
                                try:
                                    idpost = x['idpost']
                                    link = x['link']
                                    uid = link.split('https://www.facebook.com/')[1].split('_')[1]
                                except:
                                    countdown(4)
                                ...
                                if like == False:
                                    ...
                                    if 'mess' in nhan:
                                        ...
                                        if dem % doinick == 0:
                                            print(red+"-"*70)
                                            print(f'{lam}Số Xu Hiện Tại: {vang}{xu} {red}| {lam}Số Tài Khoản Facebook Đang Live {vang}{len(listck)}')
                                            ptool = 1
                                            break
                                        if dem % nvblock == 0:
                                            chongblock(delaybl)
                ...
                if nvlike2 == 1:
                    listlike2 = getjob(cookie, '')
                    if listlike2 == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Like Xu Cao              ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                        nvlike2 = 0
                    else:
                        listlike2 = listlike2
                        if 'error' in listlike2:
                            print(f'{luc}Hết Nhiệm Vụ Like Xu Cao                             ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                            nvlike2 = 0
                        else:
                            print(f'{luc}Tìm Thấy {vang}{len(listlike2)} {luc}Nhiệm Vụ Like Xu Cao                       ', end = '\r')
                            for x in listlike2:
                                try:
                                    idpost = x['idpost']
                                    link = x['link']
                                ...
                                if like == False:
                                    ...
                                    nhan = requests.post('https://tuongtaccheo.com/kiemtien/nhantien.php',headers=headers, data=data).json()
                                    if 'mess' in nhan:
                                        headers = {
                                            "Host": "tuongtaccheo.com",
                                            "x-requested-with": "XMLHttpRequest",
                                        ...
                                        dem+=1
                                        time = datetime.now().strftime("%H:%M:%S")
                                        print(f'{red}[{trang}{dem}{red}][{lam}{time}{red}][{vang}LIKEXUCAO{red}][{trang}{uid}{red}][{luc}+700{red}][{vang}{xu}{red}]')
                                        loilike = 0
                                        if dem % doinick == 0:
                                            print(red+"-"*70)
                                            print(f'{lam}Số Xu Hiện Tại: {vang}{xu} {red}| {lam}Số Tài Khoản Facebook Đang Live {vang}{len(listck)}')
                ...
                if nvlike3 == 1:
                    listlike3 = getjob(cookie, 'likepostcheo')
                    if listlike3 == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Like Xu Thường             ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                        nvlike3 = 0
                    else:
                        listlike3 = listlike3
                        if 'error' in listlike3:
                            print(f'{luc}Hết Nhiệm Vụ Like Xu Thường                            ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                            nvlike3 = 0
                        else:
                            print(f'{luc}Tìm Thấy {vang}{len(listlike3)} {luc}Nhiệm Vụ Like Xu Thường                      ', end = '\r')
                            for x in listlike3:
                                try:
                                    idpost = x['idpost']
                                    link = x['link']
                                ...
                                if like == False:
                                    ...
                                    if 'mess' in nhan:
                                        ...
                                        dem+=1
                                        time = datetime.now().strftime("%H:%M:%S")
                                        print(f'{red}[{trang}{dem}{red}][{lam}{time}{red}][{vang}LIKEXUTHUONG{red}][{trang}{uid}{red}][{luc}+300{red}][{vang}{xu}{red}]')
                                        loilike = 0
                                        if dem % doinick == 0:
                                            print(red+"-"*70)
                                            print(f'{lam}Số Xu Hiện Tại: {vang}{xu} {red}| {lam}Số Tài Khoản Facebook Đang Live {vang}{len(listck)}')
                ...
                if nvcx1 == 1:
                    ...
                    if listcx == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Cảm Xúc Vip              ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                        nvcx1 = 0
                    else:
                        listcx = listcx
                        if len(listcx) == 0:
                        ...
                        if 'error' in listcx:
                            ...
                            print(f'{luc}Tìm Thấy {vang}{len(listcx)} {luc}Nhiệm Vụ Cảm Xúc Vip                       ', end = '\r')
                            for x in listcx:
                                try:
                                    idpost = x['idpost']
                                    link = x['link']
                ...
                if nvcx2 == 1:
                    listcx2 = getjob(cookie, 'camxuccheo')
                    if listcx2 == False:
                        ...
                        if 'error' in listcx2:
                            ...
                            print(f'{luc}Tìm Thấy {vang}{len(listcx2)} {luc}Nhiệm Vụ Cảm Xúc                      ', end = '\r')
                            for x in listcx2:
                                try:
                                    idpost = x['idpost']
                                    link = x['link']
                ...
                if nvcxcmt == 1:
                    ...
                    if listcxcmt == False:
                        ...
                        if len(listcxcmt) == 0:
                            print(f'{luc}Hết Nhiệm Vụ Cảm Xúc                            ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                            nvcxcmt = 0
                        if 'error' in listcxcmt:
                            print(f'{luc}Hết Nhiệm Vụ Cảm Xúc                            ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                            nvcxcmt = 0
                        else:
                            print(f'{luc}Tìm Thấy {vang}{len(listcxcmt)} {luc}Nhiệm Vụ Cảm Xúc                      ', end = '\r')
                            for x in listcxcmt:
                                try:
                                    idpost = x['idpost']
                                    link = x['link']
                ...
                if nvfl == 1:
                    ...
                    if listfl == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Follow              ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                        nvfl = 0
                    else:
                        listfl = listfl
                        if len(listfl) == 0:
                        ...
                        if 'error' in listfl:
                            ...
                            print(f'{luc}Tìm Thấy {vang}{len(listfl)} {luc}Nhiệm Vụ Follow                      ', end = '\r')
                            for x in listfl:
                                try:
                                    idpost = x['idpost']
                                    link = x['link']
                                    ...
                                    countdown(4)
                                like = _Follow(ck, _info[1], _info[0], idpost)
                                if like == False:
                                    print(f"{luc}FAIL {lam}FOLLOW{trang}: {vang}{idpost}             ", end = '\r'); sleep(2); print('                                                       ', end = '\r')
                                    _Bypass(ck, _info[1], _info[0])
                                    loifl += 1
                                    countdown(3)
                                else:
                                    ...
                                    if 'mess' in nhan:
                                        ...
                                        dem+=1
                                        time = datetime.now().strftime("%H:%M:%S")
                                        print(f'{red}[{trang}{dem}{red}][{lam}{time}{red}][{vang}FOLLOW{red}][{trang}{idpost}{red}][{luc}+700{red}][{vang}{xu}{red}]')
                                        loifl = 0
                                        if dem % doinick == 0:
                                            print(red+"-"*70)
                                            print(f'{lam}Số Xu Hiện Tại: {vang}{xu} {red}| {lam}Số Tài Khoản Facebook Đang Live {vang}{len(listck)}')
                ...
                if nvcmt == 1:
                    ...
                    if cmtcheo == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Cmt Chéo              ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                        nvcmt = 0
                    else:
                        cmtcheo = cmtcheo
                        if len(cmtcheo) == 0:
                            print(f'{luc}Hết Nhiệm Vụ Cmt Chéo                            ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                            nvcmt = 0
                        if 'error' in cmtcheo:
                            print(f'{luc}Hết Nhiệm Vụ Cmt Chéo                            ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                            nvcmt = 0
                        else:
                            print(f'{luc}Tìm Thấy {vang}{len(cmtcheo)} {luc}Nhiệm Vụ Cmt Chéo                      ', end = '\r')
                            for x in cmtcheo:
                                try:
                                    idpost = x['idpost']
                                    link = x['link']
                                    ndcmt = json.loads(x["nd"])[0]
                                    uid = idpost.split('_')[0] if '_' in idpost else idpost
                                except:
                                    countdown(4)
                                like = CMT(ck, idpost, _info[1], _info[0], ndcmt)
                                if like == False:
                ...
                if nvshare == 1:
                    ...
                    if listshare == False:
                        print(f'{luc}Không Get Được Nhiệm Vụ Share              ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                        nvshare = 0
                    else:
                        listshare = listshare
                        if len(listshare) == 0:
                            ...
                            nvshare = 0
                        if 'error' in listshare:
                            print(f'{luc}Hết Nhiệm Vụ Share                            ', end = '\r');sleep(2); print('                                                        ', end = '\r')
                            nvshare = 0
                        else:
                            print(f'{luc}Tìm Thấy {vang}{len(listshare)} {luc}Nhiệm Vụ Share                      ', end = '\r')
                            for x in listshare:
                                try:
                                    idpost = x['idpost']
                                    link = x['link']
                                    ...
                                    countdown(4)
                                share = _Share(ck, _info[1], _info[0], idpost)
                                if share == False:
                ...
                if loilike + loicamxuc + loicxcmt + loifl + loicmt + loishare == 0:
...
if __name__ == '__main__':
    server = control_server('open')
    if 'Máy chủ đã được mở, bạn có thể chạy tool sau 5s' in server:
        print(f'{luc}{server}')
        countdown(5)
        os.system('cls' if os.name=='nt' else 'clear')
        Main()
    elif 'Máy chủ đã đóng, admin đang nấp cấp tool' in server:
        print(f'{luc}{server}')
        quit()
    else:
        print(f'{luc}{server}')
